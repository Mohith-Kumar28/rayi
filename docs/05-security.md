...
| **Step-up bound to purpose AND resource** — ✅ BUILT | A grant that says only "this user confirmed something" is a bearer capability: confirm a harmless action, spend it on a dangerous one. That is the bulk-approve hole, generalised |
| **Role ceiling on invitations and role changes** — ✅ BUILT | Better Auth's docs: "there's no built-in restriction preventing an admin from inviting someone as owner." Nobody may grant above their own level, self-promotion is refused outright, and a downgrade revokes sessions and money authority |
